    ...
    switch (cmd->type) {
        ...
        case ' ':
            ...
            if(execvp(ecmd->argv[0],ecmd->argv) < 0){
                fprintf(stderr, "execv error happend\n");
                _exit(0);
            }
            break;
        ...
        case '<':
            ...
            r = open(rcmd->file, rcmd->flags, 0664);
	    if(r == -1) {
		    fprintf(stderr, "redir error happend\n");
		    _exit(0);
	    }
	    dup2(r, rcmd->fd);
	    close(r);
	    ...
	    if (pipe(p) == -1) {
		    fprintf(stderr, "pipe error happend\n");
		    _exit(0);
	    }
	    if (!fork1()) {
		    close(STDOUT_FILENO);
...
int getcmd(char *buf, int nbuf) {
    if (isatty(fileno(stdin))) fprintf(stdout, "2019OS$ ");
    memset(buf, 0, nbuf);
